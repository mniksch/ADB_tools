...
def make_main_report(per_stats, outf, by_hs, con, acc, enr, verbose):
    ...
    basic = create_basic_summary(con, per_stats, by_hs)
    basic_summary_sheet(workbook, basic, by_hs)

    ba_basic = create_basic_summary(con, per_stats, by_hs, ba_only=True)
    basic_summary_sheet(workbook, ba_basic, by_hs, ba_only=True)

    create_cohort_grads(workbook, con, per_stats, by_hs)

    create_cohort_overviews(workbook, con, per_stats, by_hs, verbose,
                            details=True)
